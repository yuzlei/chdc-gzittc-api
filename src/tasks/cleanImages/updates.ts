...
export default async (): Promise<void> => {
    try {
        ...
        const allContentImages: Array<string> = (await ContentModel.find().distinct("content")).map((item: string) => (item.match(/<img[^>]+src="([^">]+)"/g) as Array<string>).map((str: string) => path.basename(str))).flat(2);
